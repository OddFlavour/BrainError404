...
public class CheapestFlightsWithinKStops {
    ...
    private void rec(int[][] flights, int curr, int totalCost, int count, boolean[] seen) {
        if (count > K + 1) {
            return;
        }
        if (curr == dst) {
        ...
        for (int[] f : flights) {
            ...
            if (f[0] == curr && !seen[f[1]]) {
                ...
                rec(flights, f[1], totalCost + f[2], count + 1, seen);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testNs.length; i++) {
            ...
            System.out.println(s.findCheapestPrice(testNs[i], testEdges[i], testSrcs[i], testDsts[i], testKs[i]));
        }
    }
}
